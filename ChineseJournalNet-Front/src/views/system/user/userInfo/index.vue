  ...
  <d2-container type="full">
    <template slot="header">
      ...
      <el-form :model="queryParams" ref="form" size="small" :inline="true">
        ...
        <el-col :span="20">
          ...
          <el-button type="primary" icon="el-icon-upload" size="small" @click="fileUploaderVisible = true">模板导入</el-button>
    ...
    <template slot="default">
      <el-table :data="userInfoList" size="mini" empty-text="未查询到用户信息" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="linkMan" label="联系人" min-width='100px' :show-overflow-tooltip="true" align="center" />
        <el-table-column prop="userGroupName" label="用户组"  min-width='140px' :show-overflow-tooltip="true" align="center" />
        <el-table-column prop="userType" label="用户类型" min-width='100px' :show-overflow-tooltip="true" align="center">
          <template slot-scope="scope">
            <span v-for="item in $enums.value.userType" v-show="scope.row.userType == item.value" :key="item.value">{{item.label}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="status" label="用户状态" min-width='100px' :show-overflow-tooltip="true" align="center">
          <template slot-scope="scope">
            <el-tag v-if="scope.row.status" type="success">有效</el-tag>
            <el-tag v-else type="info">无效</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="paymentType" label="用户类型" min-width='100px' :show-overflow-tooltip="true" align="center">
          <template slot-scope="scope">
            <span v-for="item in $enums.value.paymentType" v-show="scope.row.paymentType == item.value" :key="item.value">{{item.label}}</span>
          </template>
        </el-table-column>
    ...
    <user-info-form ref="userInfoForm" :allGroups="allGroups" :totalRoles="roles" @callback="searchUserInfo"></user-info-form>
    <el-dialog title="文件上传"  :visible.sync="fileUploaderVisible" width="20%" :close-on-click-modal="false">
      <el-upload drag
      accept=".xls"
...
export default {
  ...
  methods: {
    ...
    handleEdit (userInfo) {
      this.$refs.userInfoForm.show(userInfo)
    },
    ...
    handleExport () {
      this.$axiosRequest('/system/user/export', this.queryParams, this.$axiosOptions('post', null, 'blob'))
        .then((res) => {
          downloadFile(res)
        })
    },
    downloadTemplate () {
      this.$axiosRequest('/system/user/downloadTemplate', {}, this.$axiosOptions('get', null, 'blob'))
        .then(res => {
          downloadFile(res)
        })
    },
